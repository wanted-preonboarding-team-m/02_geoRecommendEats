...
public class RestaurantService {
   ...
   * @param range
   * @param sort
   * @return 맛집 목록 (lat 위도, lon 경도, type 종류, workplcae_name 사업장명, road_name_address 소재지 도로명 주소,
   * lot_number_address 소재지 지번 주소, zip_code 우편번호, rate 평점, distance 거리)
   */
  ...
  public List<RestaurantRecomResDto> getRecommendRestaurant(
    ...
    List<RestaurantRecomResDto> restaurantsList = restaurantRepository.findRestaurantsWithinRange(
        minLat, minLon, maxLat, maxLon);

    /** 3. db에서 조회한 맛집과 현재 사용자 위치 거리를 계산해서 저장 */
    for (RestaurantRecomResDto restaurant : restaurantsList) {
      double distance = findDistance(Double.parseDouble(lat), Double.parseDouble(lon),
  ...
  public RestaurantDetailResDto getRestaurantDetail(Long restaurantId) {
    return restaurantRepository.findRestaurantById(restaurantId)
        .orElseThrow(
            () -> new BusinessException(restaurantId, "restaurantId", RESTAURANT_NOT_FOUND));
  }
}
